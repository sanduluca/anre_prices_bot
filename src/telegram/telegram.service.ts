...
export class TelegramService implements OnModuleInit {
    ...
    constructor(private configService: ConfigService, private anreService: AnreService) {
        ...
        this.bot.on(message('text'), (ctx) => {
            ...
            switch (msg.text) {
                ...
                case labels.REMIND_DAILY:
                    ...
                    this.bot.telegram.sendMessage(chatId, 'You will now receive daily updates!', {
                        reply_markup: { keyboard: this.getKeyboard(true), resize_keyboard: true },
                    });
                    break;
                ...
                case labels.REMOVE_REMINDER:
                    ...
                    this.bot.telegram.sendMessage(chatId, 'Daily reminders disabled.', {
                        reply_markup: { keyboard: this.getKeyboard(false), resize_keyboard: true },
                    });
                    break;

                case labels.CONTACT_DEVELOPER:
                    this.bot.telegram.sendMessage(chatId, '<b>Developer:</b> Sandu Luca\n<a href="https://www.linkedin.com/in/sandu-luca-372439184/">LinkedIn</a>\n<a href="https://github.com/sanduluca">GitHub</a>\n<a href="mailto:sandulucawork@gmail.com">Email: sandulucawork@gmail.com</a>', {
                        parse_mode: 'HTML',
                    });
                    break;

                default:
                    this.bot.telegram.sendMessage(chatId, 'Unknown command. Please use the keyboard actions.');
            }
        });
    }
    ...
    private getKeyboard(isSubscribed: boolean) {
        ...
        return [...defaultButtons, subscriptionButton, [{ text: '👨🏻‍💻 Contact the developer' }]];
    }
    ...
    private sendPetrolPrice(chatId: number) {
        ...
        this.anreService.getPetrolPrice(yesterday, today).then((data) => {
            ...
            const diff = Number((Number(latest.price) - Number(previous.price)).toFixed(2));
            const emoji = diff < 0 ? '📉' : '📈';
            const parsedDate = dayjs(latest.date).format('DD.MM.YYYY');

            this.bot.telegram.sendMessage(chatId, `${emoji} Petrol: ${parsedDate} ${latest.price} (${diff})`);
        });
    }
    ...
    private sendPetrolPriceTable(chatId: number) {
        ...
        this.anreService.getPetrolPrice(from, to).then((data) => {
            const table = data.data.map((row) => `${dayjs(row[0]).format('DD.MM.YYYY')}\t${row[1]}`);
            this.bot.telegram.sendMessage(chatId, `*Petrol Prices Table*\n\n\`\`\`\n${table.join('\n')}\n\`\`\``, { parse_mode: 'MarkdownV2' });
        });
    }
    ...
    private sendDieselPrice(chatId: number) {
        ...
        this.anreService.getDieselPrice(yesterday, today).then((data) => {
            ...
            this.bot.telegram.sendMessage(chatId, `${emoji} Diesel: ${parsedDate} ${latest.price} (${diff})`);
        });
    }
    ...
    private sendDieselPriceTable(chatId: number) {
        ...
        this.anreService.getDieselPrice(from, to).then((data) => {
            const table = data.data.map((row) => `${dayjs(row[0]).format('DD.MM.YYYY')}\t${row[1]}`);
            this.bot.telegram.sendMessage(chatId, `*Diesel Prices Table*\n\n\`\`\`\n${table.join('\n')}\n\`\`\``, { parse_mode: 'MarkdownV2' });
        });
    }
